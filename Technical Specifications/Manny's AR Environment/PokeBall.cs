...
// This is the code I have for pokeball throwing in AR, you can control how far you the ball depending on how fast your swipe is.
// The speed of the ball can be changed with the _throwSpeed variable, also the ball has three states _thrown, _holding_, and _curve.
// Start() is called on the first frame of the game, Update() is called after every frame
// OnTouch() is called when a player starts holding the ball, Reset() resets the ball back to in front of the camera so it can be thrown again
