...
[ExecuteInEditMode]
//Use a similar class to interact with your own map tiles server
public class YourCustomTile : GOPBFTileAsync
{

    public override string GetLayersStrings(GOLayer layer)
    {
        //Enter here the layers name
    ...
    {
        //This is the layer name for POIs
        return "pois";
    }

    public override string GetLabelsStrings()
    {
        //This is the layer name for map labels
        return "labels";
    }

    public override string GetPoisKindKey()
    {
        //This is the key for the "kind" of POIs of your map features
        return "pmap:kind";
    }

    //Use that override to parse your map labels data
    public override GOFeature EditLabelData(GOFeature goFeature)
    {
        //Map here the "name" for labels
        goFeature.name = (string)goFeature.properties["name"];

        //Map here the "kind" of your labels, use the "MapboxToKind" util to convert from string to enum
        goFeature.kind = GOEnumUtils.MapboxToKind((string)goFeature.properties["pmap:kind"]);

        //Labels Y sorting, leave it like that if you don't have a custom features sorting attribute
        Int64 sort = 0;
    ...
    //Use that override to parse your map features data
    ...
    public override string vectorUrl()
    {
        ...
        var completeUrl = baseUrl + tileurl + extension + "?key=YOUR_API_KEY";
        return completeUrl;
