...
public class MoveAvatar : MonoBehaviour {
    ...
    void Start () {
        goMap.locationManager.onOriginSet.AddListener(OnOriginSet);
        goMap.locationManager.onLocationChanged.AddListener(OnLocationChanged);
    }

    void OnOriginSet (Coordinates currentLocation) {
        lastPosition = goMap.locationManager.currentLocation.convertCoordinateToVector();
        if (goMap.useElevation)
            lastPosition = GOMap.AltitudeToPoint(lastPosition);
        transform.position = lastPosition;
    }

    void OnLocationChanged (Coordinates currentLocation) {
        currentPosition = currentLocation.convertCoordinateToVector();
        if (goMap.useElevation)
            currentPosition = GOMap.AltitudeToPoint(currentPosition);

        if (!isMoving) {
            StartCoroutine(MoveAvatarSmoothly(currentPosition));
        }
    }

    IEnumerator MoveAvatarSmoothly(Vector3 targetPosition) {
        isMoving = true;
        float duration = 5f; // Adjust the duration for the movement
